...
import { MembershipPlanRow } from "../lib/membership-plans.types";
import { useToggleMembershipPlanActive } from "../lib/membership-plans.hooks";

const baseColumns: Column<MembershipPlanRow>[] = [
  {
    key: "sort_order",
    header: "Orden",
  ...
  },
  {
    ...
    header: "Duración",
    cell: (row) => (
      <span className="text-muted-foreground">{row.duration_label}</span>
    ),
  },
];
...
export function MembershipPlansTable({
  ...
  onEditAction,
  pagination,
  emptyMessage = "Aún no hay planes de membresía. Crea el primero.",
}: {
  rows: MembershipPlanRow[];
  isLoading: boolean;
  onEditAction: (plan: MembershipPlanRow) => void;
  ...
  function handleToggle(plan: MembershipPlanRow, next: boolean) {
    toggleActive.mutate(
  ...
  const columns: Column<MembershipPlanRow>[] = [
    ...baseColumns,
    {
      key: "is_active",
      header: "Activo",
